...
/** Simula un token tal como lo emite auth-service, para confirmar que booking-service
...
class JwtVerifierTest {
    ...
    private String tokenIssuedByAuthService(String role) {
    ...
    void parsesIdEmailAndRoleFromAValidToken() {
        AuthenticatedUser user = jwtVerifier.verify(tokenIssuedByAuthService("USER"));
    ...
    void recognizesAdminRole() {
        AuthenticatedUser user = jwtVerifier.verify(tokenIssuedByAuthService("ADMIN"));
    ...
    void returnsNullForATamperedToken() {
        String token = tokenIssuedByAuthService("USER");
